...
# ==========================================
    ...
    if mode == "🔌 Live Hardware (USB)":
        if not HAS_SERIAL or not selected_port:
            ...
            st.warning("⚠️ **Hardware Required:** To run real-time inference, an Arduino must be connected via USB. Please switch to the **<u>Cloud Demo (Playback)</u>** mode from the sidebar to evaluate the AI's performance.", unsafe_allow_html=True)
            st.stop()
